<?php
	...
	$allfunction = $database -> select("function", "*");
	
	if (!empty($_GET['functionid'])){
		if (!empty($_REQUEST['act'])){
			$act = $_REQUEST['act'];
			if ($act == "deletefunction"){
				$database -> delete("function", [
						"id" => $_GET['functionid']
				]);
				alertAndLoading("功能删除成功", "function_manage.php");
			}
		}
	}
		...
		if (!empty($_REQUEST['act'])){
			...
			if ($act == "addfunction"){
				if (!empty($_POST['functionname'])){
					...
					alertAndLoading("功能添加成功", "function_manage.php");
				}
			}
		}
	}
		...
		<div class="container">
			...
			<div class="row">
				...
				<div class="span9">
					...
					<form id="edit-profile" class="form-horizontal" method="post" action="function_manage.php?act=addfunction">
						<hr/>
						<fieldset>
							<div class="control-group">
								<label class="control-label" for="input01">功能名：</label>
								<div class="controls">
									<input type="text" class="input-xlarge" name="functionname" id="input01" value="" />
								</div>
							</div>
					...
					<table class="table table-bordered table-striped">
						...
						<?php 
							if (!empty($allfunction)){
								foreach ($allfunction as $function){
									echo '<tr>
											<td style="text-align: center;">
												'.$function['name'].'
											</td>
											<td style="text-align: center;">
			                                    <a href="function_manage.php?act=deletefunction&functionid='.$function['id'].'">删除</a>
											</td>
										</tr>';
								}
							}
