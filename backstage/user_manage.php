	...
	if (!empty($_GET['userid'])){
		if (!empty($_REQUEST['act'])){
			...
			if ($act == "deleteuser"){
				...
				alertAndLoading("删除成功" , "user_manage.php");
			}elseif ($act == "black"){
				...
				alertAndLoading("添加黑名单成功", "user_manage.php");
			}elseif ($act == "white"){
				$database -> update("user", [
						"flag" => 0
				],[
				...
				alertAndLoading("添加白名单成功", "user_manage.php");
			}
		}
	}
?>
							...
							<?php 
								if (!empty($alluser)){
									foreach ($alluser as $user){
										if ($user['flag'] == 0){
											$list = "black&userid=".$user['id'].'">加入黑名单';
										}else {
											$list = "white&userid=".$user['id'].'">加入白名单';
										}
										echo '<tr>
												<td>
													'.$user['realname'].'
												...
												<td>
													<a href="user_manage.php?act='.$list.'</a>
				                                    <a href="user_manage.php?act=deleteuser&userid='.$user['id'].'">删除</a>
												</td>
											</tr>';
									}
								}
